            ...
            <v-card raised>
                ...
                <v-card-text>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mt-3">
                        ...
                        <v-text-field v-model="editedItem.name" outlined prepend-icon="mdi-details" label="User FullName" required></v-text-field>
                        <v-text-field v-model="editedItem.netid" outlined prepend-icon="mdi-details" label="User NetID" required></v-text-field>
                        <v-text-field v-model="editedItem.email" outlined prepend-icon="mdi-email" label="User Email" required></v-text-field>
                        <v-select v-model="editedItem.roles" :items="roles" item-text="name" item-value="id" multiple label="User Roles" outlined prepend-icon="mdi-fingerprint"></v-select>
                    </div>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
            ...
            <v-data-table multi-sort :loading="loadingUsers" :headers="headers" :search="search" :items="users" class="elevation-1">
                <template v-slot:item.roles="{ item }">
                    <div v-for="(role, i) in item.roles" :key="i">
                        <v-chip class="ma-2" color="success" text-color="white">{{ role.name }}</v-chip>
                    </div>
                </template>
                <template v-slot:item.action="{ item }">
                    <v-icon @click="editItem(item)" color="primary">mdi-pencil</v-icon>
                    <form :action="appUrl + '/impersonate'" method="post">
                        <input type="hidden" :value="csrfToken" name="_token">
                        <input type="hidden" :value="item.id" name="id">
                        <v-btn type="submit" class="ma-2" outlined color="primary">
