...
class Helper
{
    ...
    {
        return DB::connection('oraclecdmpvt')
            ->table('vw_semester')
            ->value('current_term');
    }
    ...
    /**
     * Get the provost information
     *
     * @return Builder|Model
     * @throws \Throwable
     */
    public static function getProvost(): Model|Builder
    {
        $query =  DB::connection('oracle_cdm')
            ->table('EMP_SUPV_DEPT_UIS')
            ->where('posn_title', 'VCHAN ACAD AFF & PROV');

        throw_if(!$query->exists(), new ModelNotFoundException("Could not find provost"));

        return $query->first();
    }

    /**
     * Check if given employee uin belongs to Dean/Director/AVC
     *
     * @param $uin
     * @return bool
     */
    public static function isDeanDirectorAVC($uin): bool
    {
        return DB::connection('oraclecdmpvt')->table('EMP_SUPV_DEPT_UIS')
            ->where('uin', $uin)
    ...
    /**
     * Find the employee details from uin.
     *
     * @param $uin
     * @return Builder|Model|null
     * @throws \Throwable
     */
    public static function getEmployeeFromUin($uin): \Illuminate\Database\Query\Builder|null|\Illuminate\Database\Eloquent\Model
    {
        $query = DB::connection('oracle_cdm')
            ->table('EMP_SUPV_DEPT_UIS')
            ->where('uin', $uin);

        throw_if(!$query->exists(), new ModelNotFoundException("Could not find record for uin $uin"));
     ...
     * @return string
     */
    public static function getPic($uin): string
    {
        $result = '';
     ...
     * @return string
     */
    public static function getNopicBase64(): string
    {
